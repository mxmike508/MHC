/**
 * CHAT MODULE v1.3 - Bob AI Platform Main Interface
 * 
 * Enhanced version with FIXED image display:
...
class ChatModule {
    constructor(sharedState, moduleLoader) {
        console.log('🚀 ChatModule v1.3 initializing...');
    ...
    async init(subModule = null) {
        console.log('Chat module v1.3 initialized with FIXED image display');
    ...
    render() {
        return `
                ...
                <div class="chat-interface" id="chatInterface" style="display: ${this.currentProjectId ? 'flex' : 'none'};">
                    ...
                    <div class="chat-bottom-area">
                        ...
                        <div class="chat-input-container">
                            <!-- Action Buttons Row -->
                            <div class="action-buttons-row" id="actionButtonsRow" style="display: ${this.currentProjectId ? 'flex' : 'none'};">
                                <button class="temp-action-btn" id="commitMemoryBtn" disabled>
                                    💾 Commit to Memory
                                </button>
                                <button class="temp-action-btn" id="clearHistoryBtn" title="Clear chat history for this project">
                                    🗑️ Clear History
                                </button>
                                <button class="temp-action-btn secondary" id="switchProjectBtn">
                                    🔄 Switch Project
                                </button>
                                <div class="project-info-inline">
                                    Project: <span id="displayProjectId">${this.currentProjectId || 'None'}</span> | 
                                    Session: <span id="displayChatId">${this.currentChatId || 'None'}</span> | 
                                    RAG: <span id="displayRagId">${this.currentRagId ? 'Enabled' : 'None'}</span>
                                </div>
                            </div>
            ...
            <style>
                ...
                .temp-action-btn:hover:not(:disabled) {
                    background: #106ebe;
                    border-color: #106ebe;
                }
                ...
                .temp-action-btn.secondary:hover {
                    background: #f5f5f5;
                }
    ...
    switchToChat(projectName) {
        ...
        // Show action buttons when switching to chat
    ...
    switchProject() {
        ...
        // Hide action buttons when switching projects
        const actionButtonsRow = document.getElementById('actionButtonsRow');
        if (actionButtonsRow) {
    ...
    cleanup() {
        ...
        console.log('Chat module v1.3 cleaned up');
    }
}
