/**
 * CONFIG MODULE v1.6 - Bob AI Platform Configuration Interface
 * BITE #6A: Advanced Settings Integration - Spacing Fix
 * 
 * NEW IN v1.5: Advanced section with API timeout and context settings
 * INCLUDES: Model Selection (Bite #2) + Database Branch (Bite #3) + Features (Bite #4) + Personas (Bite #5) + Advanced (Bite #6A)
 * 
 * Features:
 ...
 * - Advanced: API timeout, context limits, and export/import functionality
...
export default class ConfigModule {
    ...
    createConfigPanel() {
            ...
            <div id="config-panel" class="config-panel">
                ...
                <div class="config-section collapsed" id="advanced-section">
                    ...
                    <div class="config-section-content">
                        ...
                        <div class="config-item">
                            <label for="cfg-api-timeout">API Timeout (seconds)</label>
                            <input type="number" id="cfg-api-timeout" min="5" max="120" value="25">
                            <div class="config-description">
                                Maximum time to wait for AI model responses before timing out.
                            </div>
                        </div>
                        ...
                        <div class="config-item">
                            <div style="display: flex; gap: 10px; margin-bottom: 6px;">
                                <button type="button" id="export-config" class="config-btn config-export">Export Settings</button>
                                <button type="button" id="import-config" class="config-btn config-import">Import Settings</button>
                            </div>
                            <div class="config-description">
        ...
        styleElement.textContent = `
            ...
            /* Advanced section - FORCE EXACT MATCH TO OTHER SECTIONS */
            #advanced-section {
                margin-bottom: 16px !important;
            }
            
            #advanced-section .config-section-header {
            ...
            #advanced-section .config-section-title {
    ...
    async postRender() {
        ...
        console.log('✅ Config Module v1.6 ready - Panel will slide in from right (SPACING FIX)');
    }
    ...
    loadAdvancedSettings() {
        // Load advanced settings from localStorage or set defaults
        try {
            const apiTimeout = localStorage.getItem('cfg_api_timeout') || '25';
            const maxContext = localStorage.getItem('cfg_max_context') || '20';
            
            const apiTimeoutInput = document.getElementById('cfg-api-timeout');
            const maxContextInput = document.getElementById('cfg-max-context');
            
            if (apiTimeoutInput) apiTimeoutInput.value = apiTimeout;
            if (maxContextInput) maxContextInput.value = maxContext;
            
            console.log('📋 Loaded advanced settings:', {
                apiTimeout: apiTimeout,
                maxContext: maxContext
            });
        } catch (error) {
            console.warn('⚠️ Failed to load advanced settings:', error);
        }
    }
    ...
    saveAdvancedSettings() {
        ...
        try {
            ...
            if (apiTimeoutInput) {
                localStorage.setItem('cfg_api_timeout', apiTimeoutInput.value);
            }
            if (maxContextInput) {
                localStorage.setItem('cfg_max_context', maxContextInput.value);
            }
            
            console.log('💾 Saved advanced settings:', {
                apiTimeout: apiTimeoutInput?.value,
                maxContext: maxContextInput?.value
            });
        } catch (error) {
            console.warn('⚠️ Failed to save advanced settings:', error);
        }
    }
    
    setupAdvancedHandlers() {
        // Add change listeners for advanced settings
        const advancedInputs = ['cfg-api-timeout', 'cfg-max-context'];
        
        advancedInputs.forEach(inputId => {
            const input = document.getElementById(inputId);
            if (input) {
                input.addEventListener('change', (e) => {
                    const settingName = inputId.replace('cfg-', '').replace('-', '_');
                    console.log(`🔧 Advanced setting ${settingName} changed to:`, e.target.value);
                    
                    // Save immediately when changed
                    localStorage.setItem(inputId.replace('-', '_'), e.target.value);
                });
            }
        });
    ...
    exportSettings() {
        try {
            // Collect all configuration settings from localStorage
            const settings = {
                version: '1.5',
                timestamp: new Date().toISOString(),
                model_settings: {
                    cfg_default_model: localStorage.getItem('cfg_default_model') || 'claude-4-sonnet',
                    cfg_vision_model: localStorage.getItem('cfg_vision_model') || 'gpt-4o',
                    cfg_code_model: localStorage.getItem('cfg_code_model') || 'claude-4-sonnet',
                    cfg_strategy_model: localStorage.getItem('cfg_strategy_model') || 'claude-4-sonnet'
                },
                database_settings: {
                    cfg_database_branch: localStorage.getItem('cfg_database_branch') || 'main'
                },
                feature_settings: {
                    cfg_show_toasts: localStorage.getItem('cfg_show_toasts') || 'true',
                    cfg_vision_auto: localStorage.getItem('cfg_vision_auto') || 'true'
                },
                advanced_settings: {
                    cfg_api_timeout: localStorage.getItem('cfg_api_timeout') || '25',
                    cfg_max_context: localStorage.getItem('cfg_max_context') || '20'
                },
                custom_personas: JSON.parse(localStorage.getItem('custom_personas_v1') || '[]')
            };
            ...
            link.href = url;
            link.download = `bob-ai-config-${new Date().toISOString().split('T')[0]}.json`;
            ...
            console.log('📦 Exported configuration:', settings);
    ...
    validateAndImportSettings(settings) {
        try {
            ...
            this.showConfirmation(
                'This will replace all current configuration settings. Are you sure?',
                () => {
                    // Proceed with import
                    let imported = 0;
                    ...
                    if (settings.model_settings) {
                        Object.keys(settings.model_settings).forEach(key => {
                            localStorage.setItem(key, settings.model_settings[key]);
                            imported++;
                        });
                    }
                    ...
                    if (settings.database_settings) {
                        Object.keys(settings.database_settings).forEach(key => {
                            localStorage.setItem(key, settings.database_settings[key]);
                            imported++;
                        });
                    }
                    
                    // Import feature settings
                    if (settings.feature_settings) {
                        Object.keys(settings.feature_settings).forEach(key => {
                            localStorage.setItem(key, settings.feature_settings[key]);
                            imported++;
                        });
                    }
                    ...
                    if (settings.advanced_settings) {
                        Object.keys(settings.advanced_settings).forEach(key => {
                            localStorage.setItem(key, settings.advanced_settings[key]);
                            imported++;
                        });
                    }
                    
                    // Import custom personas
                    if (settings.custom_personas) {
                        localStorage.setItem('custom_personas_v1', JSON.stringify(settings.custom_personas));
                        imported++;
                    }
                    ...
                    this.showNotification(`Configuration imported successfully (${imported} settings)`, 'success');
                    console.log('📥 Imported configuration:', settings);
                }
            );
